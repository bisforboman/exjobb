...
Where \textbf{O} and \textbf{D} corresponds to the state where the stop-decision is taken and the states where collection is stopped respectively. \\

\textit{discuss the LTL formulation and argue for the reader why it expresses what I say it does.}

\subsubsection{Liveness}

A liveness property should express that \say{eventually something good happens} which for this project became that "eventually a node communicates data to the server". This didn't entirely capture what was sought from the system so also a property was added for "if a data is communicated to the server, eventually the server replies to the node". 
