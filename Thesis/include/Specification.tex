...
This chapter contains the model and the specification aspect of the work. The modeling will be explained as the iterative process it was. 
...
This section explains the usage of the decision algorithm to solve over-collection. \textit{some more text will be written here once the subsection is revised}

\subsubsection{Definition}

%A decision, or a decision procedure is an algorithm that terminates with a yes or no answer given a decision problem.\cite{decisionproceduresbook} 
...
\ldot it always terminates (termination), and
\ldot it returns "Valid" when the input is valid. (soundness)
\end{definition}
...
A wireless sensor's networks objective is to collect and analyze data from an observed source. 

% argue against overcollection
